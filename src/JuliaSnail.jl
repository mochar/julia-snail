...
function elexpr(arg::ElispPlist)
    plist = Vector(undef, length(arg.nt)*2)
    for (i, (k, v)) in enumerate(pairs(arg.nt))
        plist[i*2-1] = Symbol(":", k)
        plist[i*2] = v
    end
    elexpr(Tuple(plist))
end
...
abstract type RequestOrigin end

"Request made from evaluating an Org babel block."
struct Babel <: RequestOrigin
    params::NamedTuple
    output_file::Union{String, Nothing}
end

"Request made by evaluating code in a Julia source buffer."
struct Srcbuf <: RequestOrigin end

@enum RequestStage stage_init stage_eval stage_done

@kwdef mutable struct Request{Origin<:Union{RequestOrigin, Nothing}}
    client::TCPSocket
    id::String
    origin::Origin
    "One of stage_init, stage_eval, or stage_done."
    stage::RequestStage = stage_init
...
function eval_in_module(fully_qualified_module_name::Array{Symbol}, expr::Union{Symbol, Expr}, request::Union{Request, Nothing}=nothing)
    ...
    if request === nothing
        ...
        @with CURRENT_REQUEST => request begin
            request.stage = stage_eval
            res = Core.eval(fqm, expr)
            flush(request.stdout)
            flush(request.stderr)
            request.stage = stage_done
            if request.origin !== nothing && res !== nothing
...
using Match
import JuliaSnail: Request, CURRENT_REQUEST, elexpr, ElispKeyword, ElispPlist, stage_done

include("formats.jl")

struct EmacsDisplay <: Base.AbstractDisplay end
const EMACS = EmacsDisplay()

function send(request::Request, type::Symbol, value, meta::Union{Nothing, NamedTuple})
    el = elexpr([
        Symbol("julia-snail--response-display"),
        request.id,
        ElispKeyword(type),
        value,
        meta === nothing ? nothing : ElispPlist(meta),
        request.stage == stage_done
    ])
...
function Base.display(d::EmacsDisplay, mime::MIME, x)
    ...
    type, value, meta = try
        format(request, mime, x)
    catch err
        err isa MethodError || rethrow()
        throw(MethodError(Base.display, (d, mime, x)))
    end

    send(request, type, value, meta)
end

function Base.display(d::EmacsDisplay, x)
    request = CURRENT_REQUEST[]
    request === nothing && throw(MethodError(Base.display, (d, x)))
    
    try
        type, value, meta = format(request, nothing, x)
        send(request, type, value, meta)
    catch err
        if err isa MethodError && err.f === format
            try
                type, value, meta = format(request, MIME("text/plain"), x)
                send(request, type, value, meta)
            catch err2
                err2 isa MethodError || rethrow()
